...
## derived from 👆. shows only unmapped data in a way that makes it easy to
## identify changes when doing discovery.
...
class CanViewer:
    def __init__(self, stdscr, bus):
        ...
        for x in range(0, 8):
            self.COL_WIDTHS[f"D{x:d}"] = 3

        self.decoder = Decoder(emit_unmapped=True, enable_assertions=True)
    ...
    def draw_can_bus_message(self, msg, sorting=False):
        ...
        if not sorting:
            ...
            if new_id_added or length_changed:
                ...
                # the dict The first index is the row index, the second is a
                # copy of the CAN-Bus frame
        ...
        parsed = {}
        for sender, data_key, value in self.decoder.decode(MsgAdapter(msg)):
            parsed[data_key] = value

        # Now draw the CAN-Bus message on the terminal window
        row_ind = self.ids[key]["row"]
        self.draw_line(row_ind, 0, arbitration_id_string, color)
        self.draw_line(row_ind, self.COL_WIDTHS["ID"], parsed.get("unmapped", ""), color)
